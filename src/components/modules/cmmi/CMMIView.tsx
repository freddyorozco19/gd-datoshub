...
import {
  ...
  Folder, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const MARCO: Record<"comercial" | "proyectos" | "financiero" | "datos", MarcoArea> = {
  ...
  proyectos: {
    qppo: "CPI ≥ 0.80",
    metrica: "EV / AC (Earned Value / Actual Cost) — promedio mensual",
    modelo: "Regresión logística · Modelo 1 AUC=0.88 · Modelo 2 AUC=0.85",
    indicadores: [
      { indicador: "CPI_lag2",     construccion: "CPI de hace 2 meses — shift(2) sobre histórico",        tipo: "Sub-proceso", origen: "Calculado", relevancia: "β = −1.45 — predictor más fuerte",
        descripcion: "El predictor con mayor peso absoluto en ambos modelos (β=−1.45 en M2, β=−1.22 en M1). El historial de 2 meses es más predictivo que el estado actual porque revela si el deterioro de eficiencia de costo tiene profundidad o si es puntual. Proyectos con buen CPI hace 2 meses tienen riesgo significativamente menor aunque estén bajos ahora. El modelo lo deriva aplicando shift(2) sobre la serie histórica mensual de CPI." },
      { indicador: "CPI_lag1",     construccion: "CPI del mes anterior — shift(1) sobre histórico",        tipo: "Sub-proceso", origen: "Calculado", relevancia: "β = −1.28",
        descripcion: "Estado actual de la eficiencia de costo. Un CPI bajo el mes anterior es señal directa de que el proyecto ya tiene sobrecosto. Se calcula con shift(1) sobre la serie mensual. Su coeficiente (β=−1.28) indica que por cada unidad que baja el CPI del mes anterior, la probabilidad logarítmica de alerta aumenta 1.28 unidades, manteniendo constantes las demás variables." },
      { indicador: "VA_lag1",      construccion: "(EV − PV) del mes anterior — variación de alcance",      tipo: "Sub-proceso", origen: "Calculado", relevancia: "β = −1.28 — igual de predictivo que CPI",
        descripcion: "Variación de Alcance (Schedule Variance): cuantifica la diferencia monetaria entre el valor ganado y el valor planificado del mes anterior. Con el mismo coeficiente que CPI_lag1 (β=−1.28), confirma que no es redundante sino complementaria: mide el deterioro desde la perspectiva de valor entregado en lugar de la eficiencia relativa de costo. Un proyecto puede tener VA negativa con CPI aceptable o viceversa." },
      { indicador: "CPI_trend",    construccion: "CPI_lag1 − CPI_lag2 (derivado en el pipeline)",          tipo: "Sub-proceso", origen: "Calculado", relevancia: "β = −0.35 — dirección del cambio",
        descripcion: "Captura la dirección del cambio en la eficiencia de costo. Un CPI de 0.85 bajando (trend negativo) es fundamentalmente distinto a un CPI de 0.85 subiendo (trend positivo). El modelo lo calcula restando los dos rezagos disponibles. Aunque su coeficiente (β=−0.35) es menor que los lags, aporta información que el nivel puntual no puede dar: si el proyecto está mejorando o empeorando en costo." },
      { indicador: "VA_trend",     construccion: "VA_lag1 − VA_lag2 (derivado en el pipeline)",            tipo: "Sub-proceso", origen: "Calculado", relevancia: "β = −0.35 — tendencia de la variación",
        descripcion: "Análogo al CPI_trend pero para la variación de alcance. Detecta si la brecha monetaria entre el valor ganado y planificado está creciendo o reduciéndose. Complementa al VA_lag1 de la misma forma que CPI_trend complementa a CPI_lag1: el nivel dice dónde está el proyecto en costo, el trend dice hacia dónde va." },
      { indicador: "Mes relativo", construccion: "Fase del ciclo de vida normalizada [0–1]",               tipo: "Contexto",    origen: "Calculado", relevancia: "β = −0.62",
        descripcion: "Indica en qué fracción del ciclo de vida está el proyecto (0=inicio, 1=cierre). Es esencial para eliminar falsas alarmas: un CPI de 0.85 en la fase 30–40% de un proyecto TI es históricamente normal, pero el mismo valor en la fase 80–90% es crítico. Sin esta variable el modelo generaría alertas masivas en fases donde la ineficiencia temporal de costo es parte del patrón esperado del proceso." },
      { indicador: "Portafolio",   construccion: "DATOS Y SI / TI / CONSULTORÍA",                          tipo: "Contexto",    origen: "Input",     relevancia: "β = −0.03",
        descripcion: "Controla las diferencias estructurales de riesgo de costo entre portafolios. TI es el más volátil en eficiencia de costo, mientras que DATOS Y SI es el más predecible al cierre. Sin este control, el modelo aplicaría los mismos umbrales a portafolios con perfiles de costo históricamente distintos, generando alertas incorrectas." },
      { indicador: "Líder",        construccion: "Codificación ordinal del líder del proyecto",             tipo: "Contexto",    origen: "Input",     relevancia: "β = +0.04",
        descripcion: "Único coeficiente positivo en el Modelo 1 (+0.04), lo que significa que ciertos líderes están sistemáticamente asociados con mayor riesgo de sobrecosto. Permite al modelo separar el riesgo inherente al tipo de proyecto del riesgo atribuible a la capacidad de gestión de costos del líder. La codificación ordinal preserva el orden de desempeño histórico entre líderes." },
    ],
  },
  financiero: {
    qppo: "Utilidad ≥ 18.40%",
...
function ProyectosPanel() {
  ...
  async function handleReentrenar(file: File) {
    ...
    try {
      ...
      setReMsg({ ok: true, text: `✅ Modelos entrenados — ${json.n_proyectos} proyectos · ${json.n_obs} obs · Kickoff AUC ${m?.kickoff_auc?.toFixed(3)} · M1 AUC ${m?.modelo1_auc?.toFixed(3)}` });
      setKRes(null); setSRes(null); setInfo(null);
      setProyListo(true);
    } catch (e) {
...
const PORT_COLORS_MAP: Record<string, string> = {
  "DATOS Y SISTEMAS DE INFORMACIÓN": "#818CF8",
  "CONSULTORÍA":                     "#34D399",
  "TI":                              "#FBBF24",
};
...
function CpiCartaControl({ scopeData, ind, scope, cpiCap, metadata }: {
  ...
  const th = whiteMode ? {
    ...
    tooltipOOC: "#DC2626", tooltipOK: "#16A34A",
  } : {
    bg:           "#0d1117",
    ...
    zoneOoc:  OOC_COLOR, zoneC: ZONE_C_COLOR, zoneB: ZONE_B_COLOR, zoneA: ZONE_A_COLOR,
    clStroke:    "rgba(255,255,255,0.45)", clWidth: 0.6,
    s1Stroke:    "rgba(34,197,94,0.45)", s2Stroke: "rgba(249,115,22,0.50)", s3Stroke: "rgba(239,68,68,0.55)",
    clLabelFill: "#94A3B8", s1LabelFill: "#22C55E", s2LabelFill: "#F97316", s3LabelFill: "#EF4444",
    lineStroke:  "#38BDF8",
    tooltipBg:   "#0A0C14", tooltipBorder: "1px solid rgba(255,255,255,0.14)",
    tooltipShadow:"0 12px 40px rgba(0,0,0,0.7)",
    tooltipTitle: "#F1F5F9", tooltipMuted: "#64748B", tooltipDate: "#CBD5E1",
    tooltipOOC: "#F87171", tooltipOK: "#34D399",
  };
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3px 12px" }}>
          <span style={{ color: th.tooltipMuted }}>Portafolio</span>
          <span style={{ color: whiteMode ? "#334155" : (PORT_COLORS_MAP[d.portafolio] ?? "#94A3B8") }}>{d.portafolio || "—"}</span>
          <span style={{ color: th.tooltipMuted }}>Mes relativo</span>
          <span style={{ color: th.tooltipDate }}>{(d.x * 100).toFixed(0)}%</span>
          <span style={{ color: th.tooltipMuted }}>{yLabel}</span>
          <span style={{ color: fuera ? th.tooltipOOC : th.tooltipOK, fontWeight: 700 }}>{d.y.toFixed(4)}</span>
        ...
        {fuera && <p style={{ color: th.tooltipOOC, marginTop: 8, fontSize: 11, borderTop: `1px solid ${whiteMode ? "rgba(220,38,38,0.2)" : "rgba(239,68,68,0.2)"}`, paddingTop: 6 }}>⚠ Punto fuera de límites de control (±3σ)</p>}
      </div>
    );
  }, [yLabel, whiteMode]);
      ...
      <ResponsiveContainer width="100%" height={480}>
        <ComposedChart data={visibleData} margin={{ top: 8, right: 114, bottom: 44, left: 8 }}>
          ...
          <XAxis
            ...
            tick={{ fill: th.axisTickFill, fontSize: 10, fontWeight: 300 }}
            tickLine={false} axisLine={{ stroke: th.axisLineStroke }}
          ...
          <ReferenceLine y={UCL} stroke={th.s3Stroke} strokeDasharray="6 5" strokeWidth={whiteMode ? 1 : 0.5}
            label={{ value: `+3σ = ${fmt(UCL)}`, position: "right", fill: th.s3LabelFill, fontSize: 9, dx: 6, fontWeight: whiteMode ? 600 : 400 }} />
          <ReferenceLine y={s2u} stroke={th.s2Stroke} strokeDasharray="5 5" strokeWidth={whiteMode ? 0.8 : 0.5}
            label={{ value: `+2σ = ${fmt(s2u)}`, position: "right", fill: th.s2LabelFill, fontSize: 9, dx: 6 }} />
          <ReferenceLine y={s1u} stroke={th.s1Stroke} strokeDasharray={whiteMode ? "3 4" : "5 5"} strokeWidth={whiteMode ? 0.8 : 0.5}
            label={{ value: `+1σ = ${fmt(s1u)}`, position: "right", fill: th.s1LabelFill, fontSize: 9, dx: 6 }} />
          <ReferenceLine y={CL} stroke={th.clStroke} strokeWidth={th.clWidth}
            label={{ value: `CL = ${fmt(CL)}`, position: "right", fill: th.clLabelFill, fontSize: 9, dx: 6, fontWeight: whiteMode ? 700 : 400 }} />
          <ReferenceLine y={s1l} stroke={th.s1Stroke} strokeDasharray={whiteMode ? "3 4" : "5 5"} strokeWidth={whiteMode ? 0.8 : 0.5}
            label={{ value: `-1σ = ${fmt(s1l)}`, position: "right", fill: th.s1LabelFill, fontSize: 9, dx: 6 }} />
          <ReferenceLine y={s2l} stroke={th.s2Stroke} strokeDasharray="5 5" strokeWidth={whiteMode ? 0.8 : 0.5}
            label={{ value: `-2σ = ${fmt(s2l)}`, position: "right", fill: th.s2LabelFill, fontSize: 9, dx: 6 }} />
          <ReferenceLine y={LCL} stroke={th.s3Stroke} strokeDasharray="6 5" strokeWidth={whiteMode ? 1 : 0.5}
            label={{ value: `-3σ = ${fmt(LCL)}`, position: "right", fill: th.s3LabelFill, fontSize: 9, dx: 6, fontWeight: whiteMode ? 600 : 400 }} />
...
function CpiCerradosPanel({ data }: { data: any }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        const señales: { regla: string; ind: string; idx: number; valor: number; desc: string }[] =
          data?.señales ?? [];
        if (!señales.length) return null;
        const REGLA_CLS: Record<string, string> = {
          R1: "bg-rose-500/15 text-rose-400 border-rose-500/30",
          R2: "bg-amber-500/15 text-amber-400 border-amber-500/30",
          R6: "bg-orange-500/15 text-orange-400 border-orange-500/30",
        };
        return (
          <div className="bg-white/[0.04] rounded-xl border border-white/[0.08] overflow-hidden">
            <div className="px-5 py-3 border-b border-white/[0.07] flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-slate-200">
                Señales detectadas <span className="text-slate-500 font-normal">({señales.length})</span>
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-white/[0.07]">
                    {["REGLA","IND","PROYECTO #","VALOR","DESCRIPCIÓN"].map(h => (
                      <th key={h} className="px-4 py-2.5 text-left font-medium uppercase tracking-wider text-slate-500">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {señales.map((s, i) => (
                    <tr key={i} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-2.5">
                        <span className={`px-2 py-0.5 rounded border text-[11px] font-bold ${REGLA_CLS[s.regla] ?? "bg-white/[0.05] text-slate-400 border-white/[0.08]"}`}>
                          {s.regla}
                        </span>
                      </td>
                      <td className="px-4 py-2.5 font-medium text-slate-300">{s.ind}</td>
                      <td className="px-4 py-2.5 text-slate-400 tabular-nums">{s.idx}</td>
                      <td className="px-4 py-2.5 text-slate-400 tabular-nums">{s.valor}</td>
                      <td className="px-4 py-2.5 text-slate-500">{s.desc}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
type GhEntry = { name: string; download_url: string | null; size: number; type: "file" | "dir"; url: string };

function GhFileBrowser({ apiUrl, label, onFile, uploading, downloading, setDownloading, setError }: {
  apiUrl: string; label: string;
  onFile: (f: File) => void; uploading: boolean;
  downloading: string | null;
  setDownloading: (v: string | null) => void;
  setError: (v: string | null) => void;
}) {
  const [stack, setStack]     = useState<{ url: string; label: string }[]>([{ url: apiUrl, label }]);
  const [entries, setEntries] = useState<GhEntry[]>([]);
  const [loading, setLoading] = useState(false);
  const ghIcon = <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M12 .5C5.65.5.5 5.65.5 12c0 5.1 3.3 9.42 7.88 10.95.58.1.79-.25.79-.55v-2.02c-3.2.7-3.88-1.54-3.88-1.54-.52-1.33-1.28-1.69-1.28-1.69-1.04-.71.08-.7.08-.7 1.15.08 1.76 1.18 1.76 1.18 1.02 1.75 2.68 1.24 3.34.95.1-.74.4-1.24.72-1.53-2.55-.29-5.23-1.27-5.23-5.67 0-1.25.45-2.28 1.18-3.08-.12-.29-.51-1.46.11-3.04 0 0 .97-.31 3.17 1.18a11.02 11.02 0 0 1 5.78 0c2.2-1.49 3.17-1.18 3.17-1.18.62 1.58.23 2.75.11 3.04.73.8 1.18 1.83 1.18 3.08 0 4.41-2.69 5.38-5.25 5.66.41.36.78 1.06.78 2.13v3.17c0 .3.2.66.8.55A10.51 10.51 0 0 0 23.5 12C23.5 5.65 18.35.5 12 .5z"/></svg>;
  ...
  useEffect(() => {
    ...
    fetch(current.url)
      .then(r => r.json())
      .then((j: GhEntry[]) => { if (!cancelled) setEntries(Array.isArray(j) ? j : []); })
      .catch(() => { if (!cancelled) setError("No se pudo conectar con GitHub."); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [current.url]);

  async function pickFile(f: GhEntry) {
    ...
    try {
      ...
      setError(`No se pudo descargar ${f.name}.`);
    } finally { setDownloading(null); }
  }

  const dirs  = entries.filter(e => e.type === "dir");
  const files = entries.filter(e => e.type === "file" && e.name.endsWith(".xlsx"));
  const breadcrumb = stack.map(s => s.label);

  return (
    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] overflow-hidden">
      {/* Breadcrumb */}
      <div className="px-4 py-2.5 border-b border-white/[0.06] flex items-center gap-1 flex-wrap text-xs text-slate-500">
        {ghIcon}
        {stack.map((s, i) => (
          <span key={i} className="flex items-center gap-1">
            {i > 0 && <span className="text-slate-700">/</span>}
            {i < stack.length - 1
              ? <button onClick={() => setStack(stack.slice(0, i + 1))} className="text-indigo-400 hover:text-indigo-300 transition-colors">{s.label}</button>
      ...
      {/* Botón volver */}
      {stack.length > 1 && (
        <button onClick={() => setStack(stack.slice(0, -1))}
          className="w-full flex items-center gap-2 px-4 py-2.5 border-b border-white/[0.05] hover:bg-white/[0.04] text-xs text-slate-500 hover:text-slate-300 transition-colors text-left">
          <ChevronLeft size={14} /> Volver a {breadcrumb[breadcrumb.length - 2]}
        </button>
      )}

      {loading && <p className="px-4 py-6 text-sm text-slate-500 text-center">Cargando…</p>}

      {/* Carpetas */}
      {!loading && dirs.map(d => (
        <button key={d.name} onClick={() => setStack([...stack, { url: d.url, label: d.name }])}
      ...
      {/* Archivos xlsx */}
      {!loading && files.map(f => (
        <button key={f.name} onClick={() => pickFile(f)} disabled={uploading || !!downloading}
          className="w-full flex items-center justify-between px-4 py-3 border-b border-white/[0.05] hover:bg-white/[0.04] transition-colors text-left disabled:opacity-60">
          <div className="flex items-center gap-3">
      ...
      {!loading && dirs.length === 0 && files.length === 0 && (
        <p className="px-4 py-6 text-sm text-slate-500 text-center">No hay carpetas ni archivos .xlsx aquí.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <button onClick={() => setMode("github")} className={mode === "github" ? btnActive : btnIdle}>
          {ghIcon} Desde GitHub
        </button>
      </div>
      ...
      {mode === "upload" && (
        ...
        >
          ...
          <p className="text-sm font-semibold text-slate-300">{uploading ? "Procesando…" : "Arrastra el Excel aquí"}</p>
          <p className="text-xs text-slate-500">o haz clic para seleccionar · .xlsx / .xls</p>
          <input ref={inputRef} type="file" accept=".xlsx,.xls" className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); e.target.value = ""; }} />
        </div>
      ...
      {mode === "github" && (
        <GhFileBrowser
          apiUrl={GH_PROYECTOS_API}
          label="data/Proyectos"
          onFile={onFile}
          uploading={uploading}
      ...
      {ghError && (
        <div className="flex items-start gap-2 rounded-lg bg-rose-500/10 border border-rose-500/20 px-4 py-3 text-sm text-rose-400">
          <AlertCircle size={16} className="shrink-0 mt-0.5" /> {ghError}
        </div>
      )}
...
function FinancieroPanel() {
  ...
  async function computeLineasBase() {
    setLbRes(null); setError(null); setLbLoading(true);
    try {
      const form = new FormData();
      if (finRawFile) form.append("file", finRawFile, finRawFile.name);
