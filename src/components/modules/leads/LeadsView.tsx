...
import {
  ...
  TrendingUp, Users, Clock, DollarSign, AlertCircle, Loader2,
...
// Usa startsWith para ser resistente a variaciones de encoding/acentos
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 px-6 py-3 border-b border-slate-100 bg-slate-50">
          {(
            ...
            ] as { label: string; val: string; set: (v: string) => void; opts: string[] }[]
          ...
          {anyActive && (
            <button
              onClick={() => { setMLinea("ALL"); setMComercial("ALL"); setMEstado("ALL"); }}
        ...
        <div className="overflow-auto flex-1">
          {mFiltered.length === 0 ? (
            <div className="flex items-center justify-center py-16 text-slate-400 text-sm">
              Sin leads con los filtros seleccionados
            </div>
          ) : (
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-slate-50 border-b border-slate-200">
                <tr>
                  {[
                    "Nombre", "Cliente", "Comercial", "Línea",
                    "Etapa", "Tipo Oportunidad", "Preventa",
                    "Ingresos Esp.", "Estado",
                  ].map((h) => (
                    <th
                      key={h}
                ...
                {mFiltered.map((lead) => (
                  <tr key={lead.id} className="hover:bg-blue-50/40 transition-colors">
                    <td className="px-4 py-3">
                      <p className="font-medium text-slate-800 max-w-[180px] truncate" title={lead.nombre}>
                        {lead.nombre}
                      </p>
                      {lead.correo && (
                        <p className="text-slate-400 truncate max-w-[180px] text-[10px]">{lead.correo}</p>
                      )}
                    </td>
                    <td className="px-4 py-3 text-slate-600 max-w-[140px]">
                      <span className="truncate block" title={lead.cliente}>{lead.cliente || "—"}</span>
                    </td>
                    <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{lead.comercial || "—"}</td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      {lead.linea
                        ? <span className="px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-700 font-medium">{lead.linea}</span>
                        : "—"}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`px-2 py-0.5 rounded-full font-medium ${ETAPA_STYLE[lead.etapa] ?? "bg-slate-100 text-slate-600"}`}>
                        {lead.etapa || "—"}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right font-medium text-slate-700 whitespace-nowrap">
                      {lead.ingresosEsperados ? COP(lead.ingresosEsperados) : "—"}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`px-2 py-0.5 rounded-full font-medium ${WON_STYLE[lead.ganado] ?? "bg-slate-100 text-slate-600"}`}>
                        {lead.ganado}
                      </span>
                    </td>
                  </tr>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-slate-200">
          <div className="flex-1 min-w-0 pr-4">
            ...
            <h2
              className="font-semibold text-slate-800 text-base leading-snug"
              title={lead.nombre}
            >
              {lead.nombre}
            </h2>
            <div className="flex items-center gap-1.5 mt-2 flex-wrap">
              ...
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${WON_STYLE[lead.ganado] ?? "bg-slate-100 text-slate-600"}`}>
                {lead.ganado}
              </span>
              {lead.etapa && (
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${ETAPA_STYLE[lead.etapa] ?? "bg-slate-100 text-slate-600"}`}>
                  {lead.etapa}
                </span>
              )}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-6">
          ...
          {/* Cliente & Contacto */}
          <section>
            <h3 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-3 pb-1.5 border-b border-slate-100">
              Cliente & Contacto
            </h3>
          ...
          {(lead.alcance || lead.objeto) && (
            <section>
              ...
              <div className="space-y-3">
                {lead.alcance && (
                  <div>
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Alcance</p>
                    <p className="text-xs text-slate-700 leading-relaxed">{lead.alcance}</p>
                  </div>
                )}
                {lead.objeto && (
                  <div>
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Objeto</p>
                    <p className="text-xs text-slate-700 leading-relaxed">{lead.objeto}</p>
                  </div>
                )}
              </div>
            </section>
...
function RecentLeadsWidget({ leads }: { leads: Lead[] }) {
  ...
  /* líneas que existen en los datos, ordenadas alfabéticamente */
  const availableLineas = useMemo(
    () => Array.from(new Set(leads.map((l) => l.linea).filter(Boolean))).sort(),
    [leads]
  );

  /* inicializar filtro a la línea "DATOS Y SISTEMAS…" la primera vez que llegan datos */
  useEffect(() => {
  ...
  function toggleLinea(linea: string) {
    setLineaFilter((prev) => {
      if (prev.includes(linea)) {
        if (prev.length === 1) return prev; // siempre al menos una activa
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            return (
              <button
                ...
                className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full transition-colors truncate max-w-[96px] ${
                  active
                    ? `${palette.badge} ring-1 ring-current`
      ...
      {/* lista — cada fila abre el detalle del lead */}
      {recent.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {recent.map((lead, i) => {
            ...
            const hora   = lead.fechaCreacion && lead.fechaCreacion.length >= 16
              ? lead.fechaCreacion.substring(11, 16)
              : "";
            return (
              <button
                key={lead.id}
              ...
              >
                <span className="text-[10px] font-bold text-slate-300 mt-0.5 w-3.5 shrink-0 text-right group-hover:text-violet-400 transition-colors">
                  {i + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <p
                    className="text-xs font-medium text-slate-800 truncate leading-snug group-hover:text-violet-700 transition-colors"
                    title={lead.nombre}
                  >
                    {lead.nombre}
                  </p>
                  ...
                  <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                    {lead.linea && (
                      <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full leading-tight ${palette.badge}`}>
                        {lead.linea}
                      </span>
                    )}
                    {fecha && (
                      <span className="text-[9px] text-slate-400 leading-tight whitespace-nowrap">
                        {fecha}{hora && <> · {hora}</>}
                      </span>
                    )}
                  </div>
...
/* ── widget: Leads por día segmentados por línea ─────────────────────── */
function TodayLeadsWidget({ leads }: { leads: Lead[] }) {
  ...
  const fmtLabel = new Date(selectedDate + "T12:00:00").toLocaleDateString("es-CO", {
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="p-1.5 rounded-lg bg-blue-50 shrink-0">
            <CalendarCheck2 size={15} className="text-blue-600" />
          </div>
        ...
        <div className="flex items-center gap-1 mb-1">
          ...
          <button
            ...
            disabled={isToday}
            title="Día siguiente"
            className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 disabled:opacity-30 transition-colors shrink-0"
          >
        ...
        <div className="flex items-end gap-1.5 mb-4">
          ...
          {isToday && (
            <span className="ml-auto text-xs font-medium px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-600">
              hoy
            </span>
          )}
        </div>
              ...
              return (
                <div key={linea}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-slate-600 truncate max-w-[130px]" title={linea}>
                      {linea}
                    </span>
                    <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full shrink-0 ${palette.badge}`}>
                      {count}
                    </span>
                  </div>
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${palette.bar} rounded-full transition-all duration-500`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
...
export default function LeadsView() {
  ...
  /* filtros dinámicos (opciones basadas en datos cargados) */
  const opts = useMemo(() => ({
    comercial: unique(leads.map((l) => l.comercial)),
    linea: unique(leads.map((l) => l.linea)),
  ...
  const metrics = useMemo(() => {
    ...
    const lastUpdate = filtered.reduce((max, l) => (l.ultimaModificacion > max ? l.ultimaModificacion : max), "");
  ...
  const Select = ({ label, field, options }: { label: string; field: keyof Filters; options: string[] }) => (
    <div className="flex flex-col gap-1 min-w-0">
      ...
      >
        {options.map((o) => (
          <option key={o} value={o}>
            {o === "ALL" ? "Todos" : o === "true" ? "Activo" : o === "false" ? "Inactivo" : o}
          </option>
        ))}
      </select>
    </div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Last Update", value: metrics.lastUpdate || "—", icon: Clock, color: "text-slate-600 bg-slate-100" },
            { label: "Leads", value: filtered.length.toLocaleString("es-CO"), icon: Users, color: "text-blue-600 bg-blue-50" },
            { label: "Ganados", value: metrics.ganados.toLocaleString("es-CO"), icon: TrendingUp, color: "text-emerald-600 bg-emerald-50" },
            { label: "Ingresos Esperados", value: COP(metrics.totalIngresos), icon: DollarSign, color: "text-violet-600 bg-violet-50" },
            ...
            <div key={label} className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-3">
              ...
              <div><p className="text-xs text-slate-500">{label}</p><p className="text-sm font-bold text-slate-800">{value}</p></div>
            </div>
          ))}
        </div>
        ...
        {loading && !leads.length && (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
            ...
            <p className="text-sm">Conectando a ODOO CRM…</p>
          </div>
        )}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0 bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 bg-slate-50">
                ...
                {loading && <Loader2 size={13} className="animate-spin text-blue-500" />}
              </div>
                    ...
                    {paginated.map((lead) => (
                      <tr key={lead.id} className="hover:bg-blue-50/40 transition-colors">
                        <td className="px-3 py-2.5">
                          <p className="font-medium text-slate-800 max-w-[220px] truncate" title={lead.nombre}>{lead.nombre}</p>
                          {lead.correo && <p className="text-slate-400 truncate max-w-[220px]">{lead.correo}</p>}
                        </td>
                        ...
                        <td className="px-3 py-2.5">
                          <span className={`px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${ETAPA_STYLE[lead.etapa] ?? "bg-slate-100 text-slate-600"}`}>
                            {lead.etapa || "—"}
                          </span>
                        </td>
                        ...
                        <td className="px-3 py-2.5 text-slate-500 whitespace-nowrap">
                          {lead.fechaCreacion ? lead.fechaCreacion.substring(0, 10) : "—"}
                        </td>
                        ...
                        <td className="px-3 py-2.5 text-right text-slate-600 whitespace-nowrap">
                          {lead.consultoriaCOP ? COP(lead.consultoriaCOP) : "—"}
                        </td>
                        <td className="px-3 py-2.5 text-right text-slate-600 whitespace-nowrap">
                          {lead.datosCOP ? COP(lead.datosCOP) : "—"}
                        </td>
                        <td className="px-3 py-2.5 text-right text-slate-600 whitespace-nowrap">
                          {lead.tiCOP ? COP(lead.tiCOP) : "—"}
                        </td>
                        <td className="px-3 py-2.5 text-slate-500 whitespace-nowrap">{lead.cierreEsperado || "—"}</td>
                        <td className="px-3 py-2.5">
                          <span className={`px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${WON_STYLE[lead.ganado] ?? "bg-slate-100 text-slate-600"}`}>
                            {lead.ganado}
                          </span>
                        </td>
