    ...
    <div className="flex flex-col h-full overflow-auto">
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <span className="text-sm font-semibold text-slate-700">Próximas reuniones</span>
            </div>
            <div className="divide-y divide-slate-50">
              {mockMeetings.map((m) => (
                <div key={m.id} className="flex items-center gap-4 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">{formatDate(m.start_at)}</p>
                  </div>
                  <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 shrink-0">{m.status}</span>
                </div>
              ))}
            </div>
          </div>
...
function ReunionesView() {
  const STATUS_COLORS: Record<string, string> = { programada: "bg-blue-100 text-blue-700", realizada: "bg-emerald-100 text-emerald-700", cancelada: "bg-slate-100 text-slate-500" };
  return (
    <div className="flex flex-col h-full overflow-auto">
      <Topbar title="Reuniones" subtitle="Gestión de reuniones, pendientes y seguimientos" />
      <main className="flex-1 p-6 space-y-4">
        <div className="flex justify-end">
          <button className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700 transition-colors"><Plus size={14} /> Nueva reunión</button>
        </div>
            ...
            <div key={m.id} className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-blue-50 text-blue-500 shrink-0 mt-0.5"><CalendarDays size={20} /></div>
                  <div>
                    ...
                    {m.lead && <p className="text-xs text-blue-600 mt-1.5">Lead: {m.lead}</p>}
                  </div>
                </div>
...
function IntegracionesView() {
  const STATUS_ICONS: Record<string, React.ReactNode> = {
    activa: <CheckCircle2 size={16} className="text-emerald-500" />,
    inactiva: <Minus size={16} className="text-slate-400" />,
    error: <XCircle size={16} className="text-rose-500" />,
  };
  const STATUS_COLORS: Record<string, string> = { activa: "bg-emerald-100 text-emerald-700", inactiva: "bg-slate-100 text-slate-500", error: "bg-rose-100 text-rose-700" };
  const TYPE_COLORS: Record<string, string> = { bigquery: "bg-blue-100 text-blue-700", api: "bg-violet-100 text-violet-700", csv: "bg-amber-100 text-amber-700", database: "bg-cyan-100 text-cyan-700", webhook: "bg-pink-100 text-pink-700" };
  return (
    <div className="flex flex-col h-full overflow-auto">
      <Topbar title="Integraciones" subtitle="Fuentes de datos conectadas a GD-DatosHub" />
      <main className="flex-1 p-6 space-y-4">
        <div className="flex justify-end">
          <button className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700 transition-colors"><Plus size={14} /> Nueva integración</button>
        </div>
          ...
          {mockSources.map((s) => (
            <div key={s.id} className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">{STATUS_ICONS[s.status]}<h3 className="font-semibold text-slate-800 truncate">{s.name}</h3></div>
                ...
                <button className="p-2 rounded-lg text-slate-400 hover:bg-slate-100 ml-2 shrink-0"><RefreshCw size={15} /></button>
              </div>
              {s.last_sync && <p className="text-xs text-slate-400 mt-3">Última sincronización: {formatDate(s.last_sync)}</p>}
            </div>
          ))}
        </div>
      </main>
      ...
      <main className="flex-1 p-6 space-y-6">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-700 mb-4">Leads por etapa</h3>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-700 mb-4">Ingresos esperados (M COP)</h3>
            <ResponsiveContainer width="100%" height={220}>
...
function RepositoriosView() {
  const TYPE_COLORS: Record<string, string> = { dataset: "bg-blue-100 text-blue-700", documento: "bg-amber-100 text-amber-700", reporte: "bg-violet-100 text-violet-700", pipeline: "bg-cyan-100 text-cyan-700", modelo: "bg-emerald-100 text-emerald-700" };
  return (
    <div className="flex flex-col h-full overflow-auto">
      ...
      <main className="flex-1 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar asset…" className="pl-9 pr-4 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-64" />
          </div>
          <button className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700 transition-colors"><Plus size={14} /> Agregar asset</button>
        </div>
          ...
          {mockRepos.map((r) => (
            <div key={r.id} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between">
                ...
                <button className="p-1 text-slate-400 hover:text-slate-600"><ExternalLink size={14} /></button>
              </div>
              <h3 className="font-semibold text-slate-800 mt-3">{r.name}</h3>
              <p className="text-sm text-slate-500 mt-1">{r.description}</p>
              <div className="flex flex-wrap gap-1.5 mt-3">{r.tags.map((t) => <span key={t} className="text-xs px-2 py-0.5 rounded bg-slate-100 text-slate-500">#{t}</span>)}</div>
              <p className="text-xs text-slate-400 mt-3">{r.owner}</p>
            </div>
          ))}
        </div>
      </main>
